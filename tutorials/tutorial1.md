...
2. Cliquez sur la ligne de la page Web *tutorial1.html* et observez le
  ...
  * le [code de statut](http://fr.wikipedia.org/wiki/Liste_des_codes_HTTP) de la
    réponse : 200 OK ou 304 Not modified (ou 404 Not Found) </div>

#### La console JavaScript

C'est ici que nous allons passer le reste du TD. L'onglet **Console** (caché
tout à droite) présente deux avantages :
...
3. Enfin, sélectionnez tous les éléments ayant la classe *exercice* à l'aide de
...
exercices, donc descendant d'un bloc de classe *exercice*. C'est exactement le
genre de sélection que l'on fait en CSS pour appliquer du style. Vous ne serez
donc pas surpris que JavaScript possède une fonction `document.querySelector`
qui prend un sélecteur en entrée et renvoie le premier élément correspondant. De
même, `querySelectorAll` renvoie tous les éléments correspondants.
...
Pour créer des éléments (ou nœuds), il y a principalement deux fonctions :
...
Une fois un élément créé, il faut l'insérer dans la page Web. Les fonctions à
...
permet de modifier les attributs d'un élément.
...
1. On souhaite cacher par défaut la partie facultative du formulaire
   correspondant à la liste des enfants. Rajouter `class="hidden"` comme
   attribut au `<div>` contenant la liste des enfants.  Créer la règle de style
   suivante qui a pour effet de cacher le contenu.
...
3. Pour accéder en lecture/écriture aux classes de `enfants`, nous allons
   ...
   comme un interrupteur : il active/désactive la classe selon si elle était
   désactivée/activée
   ([exemple d'utilisation](https://developer.mozilla.org/fr/docs/Web/API/Element/classList#Exemple)). Utilisez-la
   pour afficher/cacher le formulaire enfant.
